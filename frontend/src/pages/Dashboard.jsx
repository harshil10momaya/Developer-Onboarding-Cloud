  ...
  return (
    <div className="dashboard-wrapper">
      <header className="dashboard-top-header">
        <div className="header-left"><div className="search-container"><span className="search-icon">🔍</span><input type="text" placeholder="Search..." onKeyDown={(e) => { if (e.key === 'Enter') navigate('/repositories'); }} /></div></div>
        <div className="header-right">
          <div style={{ position: 'relative' }}>
            <button className="header-btn notification-btn" onClick={toggleNotif}>🔔 {unread > 0 && <span className="badge">{unread}</span>}</button>
            {showNotif && (<div style={{ position: 'absolute', right: 0, top: '40px', width: '340px', background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', boxShadow: '0 10px 40px rgba(0,0,0,0.4)', zIndex: 100, maxHeight: '400px', overflow: 'auto' }}>
              <div style={{ padding: '12px 16px', borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between' }}><strong style={{ color: '#f1f5f9', fontSize: '14px' }}>Notifications</strong></div>
              {notifications.length === 0 ? <p style={{ padding: '20px', color: '#64748b', textAlign: 'center', fontSize: '13px' }}>No notifications</p> : notifications.slice(0, 10).map((n) => (<div key={n.id} style={{ padding: '10px 16px', borderBottom: '1px solid #0f172a', cursor: n.link ? 'pointer' : 'default' }} onClick={() => n.link && navigate(n.link)}><p style={{ color: '#cbd5e1', fontSize: '13px', margin: 0 }}>{n.message}</p></div>))}
            </div>)}
          </div>
          <div className="user-profile" onClick={logout} style={{ cursor: 'pointer' }} title="Logout"><div className="avatar">👤</div><div className="user-info"><p className="user-name">{user?.full_name}</p><p className="user-role">{role}</p></div></div>
        </div>
      </header>
      <div className="dashboard-container">
        <div className="stats-cards-grid">
          <div className="stat-card" onClick={() => navigate('/repositories')} style={{ cursor: 'pointer' }}><div className="stat-icon">📊</div><div className="stat-content"><h3>Repositories</h3><p className="stat-value">{stats?.total_repositories || 0}</p></div></div>
          {role === 'developer' && <div className="stat-card" onClick={() => navigate('/progress-tracker')} style={{ cursor: 'pointer' }}><div className="stat-icon">🎬</div><div className="stat-content"><h3>Lectures Done</h3><p className="stat-value">{stats?.completed_lectures || 0}/{stats?.total_lectures || 0}</p></div></div>}
          {(role === 'mentor' || role === 'admin') && <div className="stat-card" onClick={() => navigate('/progress-tracker')} style={{ cursor: 'pointer' }}><div className="stat-icon">👥</div><div className="stat-content"><h3>Developers</h3><p className="stat-value">{stats?.active_developers || 0}</p></div></div>}
          <div className="stat-card" onClick={() => navigate('/mentor-support')} style={{ cursor: 'pointer' }}><div className="stat-icon">{stats?.pending_sessions > 0 ? '🔴' : '✅'}</div><div className="stat-content"><h3>Pending Sessions</h3><p className="stat-value">{stats?.pending_sessions || 0}</p></div></div>
          {role === 'developer' && <div className="stat-card completion-card" onClick={() => navigate('/progress-tracker')} style={{ cursor: 'pointer' }}><div className="stat-content"><h3>Completion</h3><p className="stat-value">{stats?.lectures_completion_rate || 0}%</p></div><div className="progress-circle"><div className="circle-background"><svg viewBox="0 0 100 100" className="progress-ring"><circle cx="50" cy="50" r="45" fill="none" stroke="#1f2937" strokeWidth="8" /><circle cx="50" cy="50" r="45" fill="none" stroke="#3b82f6" strokeWidth="8" strokeDasharray={`${((stats?.lectures_completion_rate || 0) / 100) * 282.7} 282.7`} strokeLinecap="round" style={{ transform: 'rotate(-90deg)', transformOrigin: '50% 50%' }} /></svg></div></div></div>}
        </div>
        {role === 'developer' && (<div className="main-content-grid"><div className="left-column"><div className="content-card onboarding-card"><h3 style={{ color: '#f1f5f9', marginBottom: '12px' }}>Your Courses</h3><div className="path-items">{courses.slice(0, 5).map((c) => (<div key={c.id} className={`path-item ${c.progress_percent === 100 ? 'completed' : c.progress_percent > 0 ? 'in-progress' : 'upcoming'}`} style={{ cursor: 'pointer' }} onClick={() => navigate(`/courses/${c.id}`)}><div className="item-marker">{c.progress_percent === 100 ? '✓' : c.progress_percent > 0 ? '◐' : '◯'}</div><span className="item-text">{c.title}</span><span className="item-status">{c.progress_percent}%</span></div>))}{courses.length === 0 && <p style={{ color: '#94a3b8' }}>No courses available.</p>}</div><button className="continue-btn" onClick={() => navigate('/learning-paths')}>Browse Learning Paths →</button></div></div>
          <div className="right-columns"><div className="content-card repositories-card"><h3>Recent Repositories</h3><div className="repositories-list">{repos.slice(0, 3).map((r) => (<div key={r.id} className="repo-item" onClick={() => navigate('/repositories')} style={{ cursor: 'pointer' }}><div className="repo-icon">📁</div><div className="repo-content"><p className="repo-name">{r.name}</p><p className="repo-stack">{(r.tech_stack || []).join(' · ')}</p></div><span className="repo-status">{r.is_analyzed ? 'Analyzed' : 'Pending'}</span></div>))}</div><button className="add-repo-btn" onClick={() => navigate('/repositories')}>View All →</button></div></div></div>)}
