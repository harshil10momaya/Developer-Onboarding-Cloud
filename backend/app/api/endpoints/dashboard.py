...
def get_dashboard_stats(
    ...
    if current_user.role in (UserRole.MENTOR, UserRole.ADMIN):
        pending = db.query(func.count(MentorSession.id)).filter(
            MentorSession.mentor_id == current_user.id, MentorSession.status == "pending"
        ).scalar() or 0
    else:
        pending = db.query(func.count(MentorSession.id)).filter(
            MentorSession.developer_id == current_user.id, MentorSession.status == "pending"
        ).scalar() or 0
